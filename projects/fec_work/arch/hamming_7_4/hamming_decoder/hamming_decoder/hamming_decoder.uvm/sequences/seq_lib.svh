class codeword_with_error_sequence extends uvm_sequence;
    ...
    virtual task body();
        ...
        if (insert_error) begin
            ...
            `uvm_info(get_name(), $sformatf("\n%07b", item_s.input_data), UVM_NONE)
        end
        finish_item(item_s);
    endtask
endclass

class set_valid_sequence extends uvm_sequence;
    `uvm_object_utils(set_valid_sequence)

    function new(string name="set_valid_sequence");
        super.new(name);
    endfunction
