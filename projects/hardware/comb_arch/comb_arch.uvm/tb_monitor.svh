...
class tb_monitor extends uvm_monitor;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            @(posedge vif.i_clock);
            {>>{item_m.input_a_data}}   = vif.i_a_data  ;
            {>>{item_m.input_b_data}}   = vif.i_b_data  ;
            item_m.incode               = vif.i_incode  ;
            item_m.opcode               = vif.i_opcode  ;
            item_m.output_data          = vif.o_data    ;
            item_m.flags                = vif.o_flags   ;
            // item_m.print();
